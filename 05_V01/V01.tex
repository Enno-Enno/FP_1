...
\begin{align}
	m_{CB}&=\qty{-0.439(0.013)}{\per\us} &\Rightarrow & & \tau&=\frac{1}{|m|}=\qty{2.28(0.07)}{\us} \\
...
\newpage
\section{Diskussion}
Trotz der fehlerhaften Daten war eine ungefähre Bestimmung der Myonenlebenszeit möglich. 
So weichen diese, durch die Ausgleichsgerade bestimmten Werte... nur um ... von der Teorie ab. 

Jedoch sind die Exponital Fits deutlich weiter vom Literaturwert entfernt. 
Dieses könnte daran liegen, dass diese von einem Signifikanten Untergrund von $U_C=\num{1.71(0.26)}$ ausgehen, 
welcher aber anscheind nicht eine so dominante Rolle für Messwerte über $\qty{1}{\us} $ spielen. 

Wenn man versucht die ungewöhnliche Kante an die anderen Messwerte anzupassen werden die Ergebnisse ... gerade für den Exponital Fit noch ungenauer. 
Da wir keinen Plan haben woher dieser kommt und die Ergebnisse schlechter werden, 
ist davon auszugehen das dieser Teil der Daten komplett unbrauchbar für die Bestimmung der Lebnsdauer ist.
Daher ist das Ergebniss der Ausgleichsgerade im Intervall von $[\qty{1}{\us}-\qty{10}{\us}]$ die beste Annäherung an den Literaturwert, der mit diesen Daten möglich ist. 
